...
contains
    ...
    subroutine TLabMPI_Trp_Initialize(inifile)
        ...
        ! I-window MUST be allocated here — BEFORE the K-shmem MPI_Win_allocate_shared below.
        ! On Cray MPICH, allocating a shared window on any MPI_Comm_split_type sub-comm (such as
        ! the K-shmem sub-comm in the next block) taints the VA allocator for ALL subsequent
        ! MPI_Win_allocate_shared calls: those windows get per-XCD VAs instead of the uniform VA
        ! that an untainted allocation gives.  APU_DIRECT's apu_all_i works correctly because its
        ! MPI_Win_allocate_shared(ims_comm_x) is the FIRST shmem allocation (before any split-type
        ! sub-comm window); we replicate that ordering here.  Confirmed by 2026-05-22 crashlog:
        ! PE 0 (XCD-0) got I-shm VA ~109 TB and PE 3 (XCD-1) got ~75 TB — same ims_comm_x window,
        ! per-XCD allocation because the K-shmem window had already run.
        if ((trp_mode_i == TLAB_MPI_TRP_APU_ASYNC .or. trp_mode_i == TLAB_MPI_TRP_FABRIC_DIRECT) .and. ims_npro_i > 1) then
            ...
            ! I-dup taken immediately after the I-window, before the K-shmem sub-comm below.
    ...
    subroutine TLabMPI_Trp_ExecI_Forward_Real(a, b, trp_plan)
        ...
        if (trp_mode_i == TLAB_MPI_TRP_APU_DIRECT) then
            ...
            ! recv buffer (apu_async_recv_i); inter-node peers via MPI ISEND/IRECV on the
            ! dup'd untainted comm apu_async_mpi_comm_i (dup of ims_comm_x taken before any
            ! MPI_Win_allocate_shared in TLabMPI_Trp_Initialize).
            ...
            !   Use local I-comm rank m on the dup'd untainted comm.
            do m = 0, ims_npro_i - 1
                if (.not. apu_async_is_local_i(m)) then
                    l = l + 1
    ...
    subroutine TLabMPI_Trp_ExecI_Backward_Real(b, a, trp_plan)
        ...
        if (trp_mode_i == TLAB_MPI_TRP_APU_DIRECT) then
            ...
            ! Two-level I-backward: same pattern as I-Forward FABRIC_DIRECT.
            ! Intra-node via !$omp target GPU writes; inter-node via apu_async_mpi_comm_i
            ! (dup of ims_comm_x taken before any MPI_Win_allocate_shared) with local rank m.
            ! MPI_Win_fence removed — it was collective on the tainted shmem subcomm.
            size = trp_plan%size3d
            call c_f_pointer(c_loc(wrk_mpi_dp(1)),         c_wrk_dp,      shape=[size])
            call c_f_pointer(c_loc(wrk_mpi_dp(size + 1)),  c_wrk_dp_recv, shape=[size])
            l = 0
