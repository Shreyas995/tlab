! vmpi_commtest_p1.f90
! Phase 1: comm = dup(ims_comm_x),  hipDevSync AFTER IRECVs  [current broken code]
! Expected: hang — ranks 12-17 stuck at [S4], ranks 36-41 stuck at [S8]
! Output: fort.1XX where XX = rank (fort.100 to fort.147)
!
...
program vmpi_commtest_p1
    ...
    integer, parameter :: FUNIT_BASE = 100   ! fort.100 + rank
    ...
    call MPI_Cart_sub(comm_xz, remain_dims, comm_x, ierr)   ! I-direction

    remain_dims(1) = .true.; remain_dims(2) = .false.
    call MPI_Cart_sub(comm_xz, remain_dims, comm_z, ierr)   ! K-direction

    ! --- Send/recv maps (identical to tlab_mpi_transpose.f90) ---
    ...
    ! --- K-direction taint source (identical to FABRIC_DIRECT init) ---
    ...
    ! --- Staging buffer: [1..CHUNK*NPRO_I]=recv, [CHUNK*NPRO_I+1..]=send ---
    ...
    ! hipDeviceSynchronize AFTER IRECVs — this is the step expected to hang
#ifdef USE_APU
    hip_err = hipDeviceSynchronize()
    write(funit,'(a,i0)') '[Ssync] hipDevSync done, err=', int(hip_err)
    flush(funit)
#endif
