! vmpi_commtest_p3.f90
! Phase 3: comm = MPI_Comm_split(color=k, key=i),  hipDevSync AFTER IRECVs
! Expected: may fix the hang — if passes, the COMMUNICATOR CHOICE alone is the fix
! Output: fort.3XX where XX = rank (fort.300 to fort.347)
!
! Build:  hipcc -c hip_write_fence.hip -o hip_write_fence.o
!         ftn -DUSE_APU vmpi_commtest_p3.f90 hip_write_fence.o -lamdhip64 -o vmpi_commtest_p3.x

program vmpi_commtest_p3
    use mpi_f08
    use iso_c_binding
    implicit none
    ...
    integer, parameter :: FUNIT_BASE = 300   ! fort.300 + rank
    ...
    type(MPI_Comm) :: comm_xz, comm_z, shmem_k, test_comm
    type(MPI_Win)  :: k_win
    ...
    ! --- Cartesian topology (to derive ims_pro_k and ims_pro_i only) ---
    ...
    ! comm_z still needed for K-shmem taint source
    ...
    ! --- Test comm: fresh MPI_Comm_split from MPI_COMM_WORLD
    !     color = ims_pro_k  -> groups all ranks with same k together
    !     key   = ims_pro_i  -> local rank in new comm = ims_pro_i (matches maps_s/maps_r)
    !     This comm has NO lineage from ims_comm_x or ims_comm_z. ---
    call MPI_Comm_split(MPI_COMM_WORLD, ims_pro_k, ims_pro_i, test_comm, ierr)
    call MPI_Comm_rank(test_comm, local_rank_check, ierr)
    write(funit,'(a,i2,a,i2,a)') '[COMM] MPI_Comm_split created, local_rank=', &
        local_rank_check, ' (should equal ims_pro_i=', ims_pro_i, ')'
    flush(funit)
    ...
    write(funit,'(a,i0)') '[Ssync] hipDevSync after IRECVs, err=', int(hip_err)
    flush(funit)
#endif
