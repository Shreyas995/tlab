...
program DNS
    ...
    do
        ...
        DNS_PROBE('LOOP:after-rk-q', q(1, 1), isize_field*inb_flow, -2)
        if (scal_on) DNS_PROBE('LOOP:after-rk-s', s(1, 1), isize_field, -2)
        ...
        DNS_PROBE('LOOP:after-filter-q', q(1, 1), isize_field*inb_flow, -2)
        ...
        ! -------------------------------------------------------------------
        call DNS_BOUNDS_CONTROL()
        DNS_PROBE('LOOP:after-bounds-q', q(1, 1), isize_field*inb_flow, -2)
        call DNS_OBS_CONTROL()
        DNS_PROBE('LOOP:after-obs-q', q(1, 1), isize_field*inb_flow, -2)
        call DNS_FILTER_CONTROL(q, s)           ! adaptive [Filter] alpha ramp (dilatation-gated)
        DNS_PROBE('LOOP:after-filtctl-q', q(1, 1), isize_field*inb_flow, -2)
        ...
        if (PhAvg%active) then
            if (mod(itime, PhAvg%stride) == 0) then
                ...
                DNS_PROBE('LOOP:after-phspace1-q', q(1, 1), isize_field*inb_flow, -2)
                call AvgPhaseSpace(wrk2d, inb_scal, itime/PhAvg%stride, nitera_first, nitera_save/PhAvg%stride, 2)
                DNS_PROBE('LOOP:after-phspace2-q', q(1, 1), isize_field*inb_flow, -2)
        ...
        DNS_PROBE('LOOP:after-phaseavg-q', q(1, 1), isize_field*inb_flow, -2)

        if (use_tower) then
            call DNS_TOWER_ACCUMULATE(q, 1, wrk1d)
            call DNS_TOWER_ACCUMULATE(s, 2, wrk1d)
            DNS_PROBE('LOOP:after-tower-q', q(1, 1), isize_field*inb_flow, -2)
        end if
        ...
        if (mod(itime - nitera_first, nitera_stats_spa) == 0) then  ! Accumulate statistics in spatially evolving cases
            ...
            DNS_PROBE('LOOP:after-statsspa-q', q(1, 1), isize_field*inb_flow, -2)
        end if
        if (mod(itime - nitera_first, nitera_stats) == 0) then      ! Calculate statistics
            ...
            DNS_PROBE('LOOP:after-stats-q', q(1, 1), isize_field*inb_flow, -2)
            if (scal_on) DNS_PROBE('LOOP:after-stats-s', s(1, 1), isize_field, -2)
        end if
        if (mod(itime - nitera_first, nitera_save) == 0 .or. &      ! Check-pointing: Save restart files
            ...
            if (flow_on) then
                ...
                DNS_PROBE('LOOP:after-saveflow-q', q(1, 1), isize_field*inb_flow, -2)
            end if
            ...
            if (scal_on) then
                ...
                DNS_PROBE('LOOP:after-savescal-s', s(1, 1), isize_field, -2)
                DNS_PROBE('LOOP:after-savescal-q', q(1, 1), isize_field*inb_flow, -2)
            end if
        ...
        DNS_PROBE('LOOP:after-save-q', q(1, 1), isize_field*inb_flow, -2)
        if (scal_on) DNS_PROBE('LOOP:after-save-s', s(1, 1), isize_field, -2)

        if (mod(itime - nitera_first, nitera_pln) == 0) then
            call PLANES_SAVE()
            DNS_PROBE('LOOP:after-planes-q', q(1, 1), isize_field*inb_flow, -2)
        end if
